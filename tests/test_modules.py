...
def test_verify_terraform(dir_name, wipes, tmp_path):
    ...
    import json
    actual_path = os.path.join(here, 'terraform', dir_name, 'actual.json')
    if os.path.exists(actual_path):
        os.unlink(actual_path)
    with open(actual_path, 'w') as fp:
        json.dump(actual, fp, indent=2)

    expected_path = os.path.join(here, 'terraform', dir_name, 'expected.json')
...
def next_id(val, ids):
    # grab the next id
    next_id = ids[_counter]

    # increment the counter
    ids[_counter] = next_id + 1

    # map the volatile id to a non-volatile id
    new_id = str(uuid.UUID(int=next_id))
    ids[val] = new_id
    return new_id
